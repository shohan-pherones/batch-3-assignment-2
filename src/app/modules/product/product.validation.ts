...
const variantValidationSchema = z.object({
  type: z
    .string()
    .min(3, { message: 'Variant type must be at least 3 characters long.' })
    .max(20, { message: 'Variant type must be at most 20 characters long.' })
    .nonempty({ message: 'Variant type is required.' }),
  value: z
    .string()
    .min(1, { message: 'Variant value must be at least 1 character long.' })
    .max(20, { message: 'Variant value must be at most 20 characters long.' })
    .nonempty({ message: 'Variant value is required.' }),
});
...
const productValidationSchema = z.object({
  ...
  tags: z.array(
    z
      ...
      .nonempty({ message: 'Each tag is required.' }),
  ),
  variants: z
